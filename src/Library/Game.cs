...
namespace Library
{
    public class Game
    {
        ...
        /// <summary>
        /// Por Expert el encargado de conocer a donde se realizaron los tiros es Game, por lo que
        /// el método que checkea si es agua o no, y suma en un total es Game
        /// </summary>
        /// <param name="CheckShots("></param>
        /// <returns>Devuelve la cantidad de tiros al agua y a barco por separado</returns>
        public (int, int) CheckShots()
        {
            int water = 0;
            int hitted = 0;
            for (int i = 0; i < BoardPlayer1.shots.Count; i += 2)
            {
                ...
                (bool hit, string ship) = BoardPlayer1.CheckShip(setter1, setter2, BoardPlayer2.shipPos);
                if (hit)
                {
                    hitted += 1;
                }
            ...
            for (int i = 0; i < BoardPlayer2.shots.Count; i += 2)
            {
                ...
                (bool hit, string ship) = BoardPlayer2.CheckShip(setter1, setter2, BoardPlayer1.shipPos);
                if (hit)
                {
                    hitted += 1;
                }
